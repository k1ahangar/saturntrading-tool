...
const INTERVAL = "1m"; // Adjusted for 30-minute trading
...
async function fetchHistoricalData(symbol) {
    try {
        const response = await fetch(`https://api.binance.com/api/v3/klines?symbol=${SYMBOLS[symbol]}&interval=1m&limit=30`);
        const data = await response.json();
        return data.map(candle => parseFloat(candle[4])); // Closing prices of last 30 minutes
    } catch (error) {
        console.error(`Error fetching historical data for ${symbol} (30M approximation):`, error);
        return [];
    }
}
...
async function updateSignals(symbol) {
    ...
    if (historicalData[symbol].length < 30) return;
...
}, 5000); // Faster updates for 30-minute trading
