...
const INTERVAL = "1d"; // Use 1-day interval and aggregate for 1 week
...
async function fetchHistoricalData(symbol) {
    try {
        const response = await fetch(`https://api.binance.com/api/v3/klines?symbol=${SYMBOLS[symbol]}&interval=1d&limit=7`);
        const data = await response.json();
        return data.map(candle => parseFloat(candle[4])); // Closing prices of last 7 days
    } catch (error) {
        console.error(`Error fetching historical data for ${symbol} (1W approximation):`, error);
        return [];
    }
}
...
function predictPrice(data) {
    ...
    let trend = (data[data.length - 1] - data[0]) / data.length; // Basic trend prediction
    return (lastClose + trend).toFixed(2);
}

function determineSignal(indicators) {
    let weight = 0;
    if (indicators.rsi > 50) weight += 2;
    if (indicators.ema10 > indicators.sma10) weight += 2;
    if (indicators.macd > 0) weight += 3;
    if (indicators.momentum > 0) weight += 2;
    
    if (weight >= 6) return { signal: "Strong Buy", power: "High Confidence" };
    if (weight >= 3) return { signal: "Buy", power: "Medium Confidence" };
    if (weight <= -6) return { signal: "Strong Sell", power: "High Confidence" };
    if (weight <= -3) return { signal: "Sell", power: "Medium Confidence" };
...
async function updateSignals(symbol) {
    ...
    if (historicalData[symbol].length < 7) return;
