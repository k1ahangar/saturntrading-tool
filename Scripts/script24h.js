...
const INTERVAL = "1h"; // Adjusted for intraday trading
...
async function fetchHistoricalData(symbol) {
    try {
        const response = await fetch(`https://api.binance.com/api/v3/klines?symbol=${SYMBOLS[symbol]}&interval=1h&limit=24`);
        const data = await response.json();
        return data.map(candle => parseFloat(candle[4])); // Closing prices of last 24 hours
    } catch (error) {
        console.error(`Error fetching historical data for ${symbol} (1D approximation):`, error);
        return [];
    }
}
...
async function updateSignals(symbol) {
    ...
    if (historicalData[symbol].length < 24) return;
...
}, 30000); // Updated interval for more frequent updates
