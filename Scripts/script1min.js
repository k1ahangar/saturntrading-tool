...
const INTERVAL = "1m"; // Adjusted for 1-minute trading
...
async function fetchHistoricalData(symbol) {
    try {
        const response = await fetch(`https://api.binance.com/api/v3/klines?symbol=${SYMBOLS[symbol]}&interval=1m&limit=60`);
        const data = await response.json();
        return data.map(candle => parseFloat(candle[4])); // Closing prices of last 60 minutes
    } catch (error) {
...
async function updateSignals(symbol) {
    ...
    if (historicalData[symbol].length < 60) return;
...
}, 1000); // Fast updates every second for 1-minute trading
