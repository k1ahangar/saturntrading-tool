...
const INTERVAL = "1w"; // Use 1-week interval and aggregate
...
async function fetchHistoricalData(symbol) {
    try {
        const response = await fetch(`https://api.binance.com/api/v3/klines?symbol=${SYMBOLS[symbol]}&interval=1w&limit=4`);
        const data = await response.json();
        return data.map(candle => parseFloat(candle[4])); // Closing prices of last 4 weeks
    } catch (error) {
        console.error(`Error fetching historical data for ${symbol} (1M approximation):`, error);
        return [];
    }
}
...
async function updateSignals(symbol) {
    ...
    if (historicalData[symbol].length < 4) return;
